...
// fix 2: blur tool
// re check the constructor and deconstructor of the filter.
ConvolutionFilter::ConvolutionFilter() : kernel_(NULL) {}

// sourced from iter1 solution
// fix 6: convolution kernel delete
// many unnecessory pointers existed even the action was ended.
// kernel (FloatMatrix pointer)  was there but never deleted
// Therefore, made sure that there was a destructor
...
// sourced from iter1 solution
// SetupFilter() initialize the kernel value. NULL.
void ConvolutionFilter::SetupFilter() { kernel_ = CreateKernel(); }

// sourced from iter1 solution
// probably did not really matter since it was about seg fault
// not calculation but rechecked again.
...
// fix 6: convolution kernel delete
// sourced fro iter1 solution
// CleanupFilter() delete the kernel_ and point to NULL
