...
// sourced from iter1 solution File
// instead of directly comparing with buffer to actual image, made another
// buffer so the change source and destination can be more effective.
void Filter::ApplyToBuffer(PixelBuffer *buffer) {
  ...
  // fix 6: convolution filter kernel
  // CleanupFilter() was a virutal function that delete the kernel_
  CleanupFilter();

  if (!can_calculate_in_place()) {
    // Moreover, after the execution, the source buffe is deleted
    delete source;
  }
}
