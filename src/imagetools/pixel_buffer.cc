...
void PixelBuffer::SaveToFile(const std::string &filename) {
  Image image = Image(this->width(), this->height(), 4);
  ColorData color_ = ColorData();
  for (int w = 0; w < this->width(); w ++)
  for (int h = 0; h < this->height(); h ++) {
    color_ = pixel(w, h);
    image.SetFloatValue(w, h, 0, color_.red());
    image.SetFloatValue(w, h, 1, color_.green());
    image.SetFloatValue(w, h, 2, color_.blue());
    image.SetFloatValue(w, h, 3, color_.alpha());
  }
...
void PixelBuffer::LoadFromFile(const std::string &filename) {
  ...
  for (int h = 0; h < image->Height(); h ++) {
    color_.set_red(image->FloatValue(w, h, 0));
    color_.set_green(image->FloatValue(w, h, 1));
    color_.set_blue(image->FloatValue(w, h, 2));
    color_.set_alpha(image->FloatValue(w, h, 3));

    this->set_pixel(w, h, color_);
  }
  delete image;
}
